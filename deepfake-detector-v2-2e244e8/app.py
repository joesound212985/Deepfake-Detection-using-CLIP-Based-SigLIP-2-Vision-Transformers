...
DETECT_MAX_VIDEO_FRAMES = int(os.getenv("DETECT_MAX_VIDEO_FRAMES", "8"))
...
if _OpenAIClient is not None and LLM_API_KEY:
    try:
...
def explain_with_llm(metrics: dict) -> str:
    """
    Use an OpenAI-compatible LLM (HF router by default, Llama 3 70B default model)
...
def extract_video_frames(video_path: str, max_frames: int = DETECT_MAX_VIDEO_FRAMES):
    """
...
class BayesianFusionV2:
    ...
    def __init__(self, calibrate=False, logistic=False):
        ...
        self.feature_names = [
            "visual", "freq", "forensic", "grain",
            "hist", "fft", "texture", "cfa", "realprior"
        ]
    ...
    def _get_reliability(self, features):
        hist = features.get("hist", 0.5)
        fft = features.get("fft", 0.5)
        grain = features.get("grain", 0.5)
        forensic = features.get("forensic", 0.5)
        reliability = {
            "visual": 1.0,
            "freq": 1.0,
            "forensic": 1.2 if forensic > 0.45 else 1.0,
            "grain": 0.5 if fft < 0.2 else 1.0,
            "hist": 1.2 if hist > 0.8 else 1.0,
            "fft": 1.0,
            "texture": 1.0,
            "cfa": 1.0,
            "realprior": 0.8,
...
class DirichletBayesianFusion:
    ...
    def fuse(self, features):
        ...
        for k, p in features.items():
            w = self.feature_weights.get(k, 1.0)
            alpha = self.evidence_from_prob(p, w)
            total_alpha += alpha
...
def _predict_single_image(image):
    ...
    with torch.inference_mode():
        ...
        # --------------------------------------------------------
        #            Bayesian fusion (multi-cue posterior)
        # --------------------------------------------------------
        ...
        fusion_features = {
            "visual": _safe_prob(visual_prob, p_global),
            "freq": _safe_prob(freq_prob, p_global),
            "forensic": _safe_prob(forensic_val, 0.5),
            "grain": _safe_prob(grain_real, 0.5),
            "hist": _safe_prob(hc_score, 0.5),
            "fft": _safe_prob(fft_conf_real, 0.5),
            "texture": _safe_prob(texture_noise, 0.5),
            "cfa": _safe_prob(cfa_fake_score, 0.5),
            "realprior": _safe_prob(real_prior_v3_val, 0.5),
        }

        certainty = abs(p_enhanced - 0.5) * 2.0
        try:
            fusion = BayesianFusionV2(calibrate=False)
            fusion_result = fusion.fuse(fusion_features, prior_fake=p_enhanced)
            p_enhanced = float(fusion_result["posterior_fake"])
        ...
        # Dirichlet evidence-based fusion (optional blend)
        try:
            ...
            # Blend Dirichlet posterior with current enhanced prob for stability
            p_enhanced = float(0.6 * p_enhanced + 0.4 * dirichlet_p)
            certainty = float(0.5 * certainty + 0.5 * max(0.0, 1.0 - dirichlet_uncertainty))
        except Exception as _e:
            print(f"[dirichlet_fusion] error: {_e}")

        # Dirichlet-driven label hint (light-touch)
        if dirichlet_p is not None:
            if dirichlet_p > 0.65 and (dirichlet_uncertainty is None or dirichlet_uncertainty < 0.25):
                label = "FAKE"
            elif dirichlet_p < 0.35 and (dirichlet_uncertainty is None or dirichlet_uncertainty < 0.25):
                label = "REAL"
            elif dirichlet_uncertainty is not None and dirichlet_uncertainty > 0.40:
                label = "INCONCLUSIVE"
        ...
        report = {
            ...
            "bayesian_fusion_posterior": float(p_enhanced),
            "bayesian_fusion_certainty": float(certainty),
        ...
        explanation = explain_with_llm(llm_metrics)
...
def predict(image):
    ...
    # Video handling: sample frames and report the highest-risk frame
    if isinstance(image, str) and _is_video_file(image):
        frames = extract_video_frames(image, max_frames=DETECT_MAX_VIDEO_FRAMES)
        if not frames:
            return "Video decode error: no frames found.", None, None, None, None, ""
        frame_results = []
        for idx, frame in enumerate(frames):
            res = _predict_single_image(frame)
            frame_results.append((idx, res))
        if not frame_results:
            return "Video decode error: failed to process frames.", None, None, None, None, ""

        def _frame_score(result_tuple):
            try:
                data = _json.loads(result_tuple[4]) if result_tuple[4] else {}
                return float(data.get("final_prob", 0.0))
            except Exception:
                return 0.0

        best_idx, best = max(frame_results, key=lambda item: _frame_score(item[1]))
        html, heatmap_img, fft_panel_img, jitter_img, report_str, explanation = best[1]

        # Aggregate frame-level scores for JSON report
        frame_probs = []
        for i, r in frame_results:
            try:
                rd = _json.loads(r[4]) if r[4] else {}
                frame_probs.append(
                    {
                        "frame_index": int(i),
                        "final_prob": rd.get("final_prob"),
                        "prediction": rd.get("prediction"),
                    }
                )
            except Exception:
                frame_probs.append({"frame_index": int(i), "final_prob": None, "prediction": None})
        ...
        report_data["video_frame_probs"] = frame_probs
        report_data["video_selected_frame"] = int(best_idx)
        report_data["video_total_sampled_frames"] = len(frame_results)
        report_str = _json.dumps(report_data, indent=2)

        html = f"[Video frame {best_idx+1}/{len(frame_results)} (highest risk)]<br>" + html
        explanation = f"(Video frame {best_idx+1}/{len(frame_results)}) " + explanation
        return html, heatmap_img, fft_panel_img, jitter_img, report_str, explanation

    # Non-video: images (includes AVIF via loader)
    return _predict_single_image(image)
...
with gr.Blocks(css=".output-image {transition:opacity 0.3s;}") as demo:
    ...
    with gr.Row():
        with gr.Column():
            image_in = gr.Image(type="filepath", label="Upload Image / Video")
            with gr.Row():
        ...
        with gr.Column():
            ...
            explanation_out = gr.Markdown(label="AI Explanation (HF LLM)")
            with gr.Row():
