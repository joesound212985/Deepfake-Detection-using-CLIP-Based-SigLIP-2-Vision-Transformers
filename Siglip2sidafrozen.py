...
class SigLIP2_MTL(nn.Module):
    def __init__(self, siglip_ckpt="google/siglip-base-patch16-224", seg_layers=(2,6,10,-1), embed_dim=256, dropout_rate=0.0):
        ...
        # --- FREEZING LOGIC ADDED HERE ---
        # Freeze patch embeddings
        for param in self.encoder.vision_model.embeddings.parameters():
            param.requires_grad = False
            
        # Freeze early blocks (up to block 20, i.e. 1-21 in 1-based indexing)
        # Leaves the final blocks (e.g., 21, 22, 23) trainable
        for i, layer in enumerate(self.encoder.vision_model.encoder.layers):
            if i < 21:
